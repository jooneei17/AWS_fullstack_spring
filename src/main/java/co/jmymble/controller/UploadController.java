...
public class UploadController {
	...
	public void upload(MultipartFile[] files) throws IllegalStateException, IOException{
		...
		for(MultipartFile m : files){
			log.info(m.getOriginalFilename()); //실제 파일의 이름
			log.info(m.getSize());
	...
	public ResponseEntity<byte[]> getFile(AttachFileDTO dto, Boolean thumb){
		...
		try {
			...
			headers.add("Content-Type", Files.probeContentType(file.toPath()));//header를 통해 응답 제어
	...
	public ResponseEntity<byte[]> download(AttachFileDTO dto){
		...
		try {
			...
			headers.add("Content-Disposition", "attachmemt; filename=" + new String(dto.getName().getBytes("utf-8"), "iso-8859-1"));
			result = new ResponseEntity<>(FileCopyUtils.copyToByteArray(file), headers, HttpStatus.OK);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	@GetMapping(value="dto", produces=MediaType.APPLICATION_JSON_VALUE) @ResponseBody //json형태로 해줘.. -> 안해줌
	...
	private boolean isImage(File file) throws IOException{
		...
		// mime image/png image/jpeg image/x-icon : 얘네들은 잘 나오는데
		// webp null: 얘는 널이 나온다...라는 문제가 있음 ===> 위에서 if문을 통해 처리
